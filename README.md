...
- 支出、收入、帳戶轉帳的新增、編輯、刪除與復原
- 客製帳戶：現金、LINE、永豐、台銀、郵局；新增、轉帳、OCR 與載具匯入都直接用按鈕選擇
...
- 口語帳戶支援 LINE、永豐、台銀、郵局與現金，也可說「從永豐轉到郵局」
- 截圖在裝置內 OCR，辨識金額、日期、商家與發票號碼後產生可確認草稿
- 財政部手機條碼載具同步，連同發票品項匯入
- 本機規則＋Gemini AI 自動分類；AI 只接收商家與品項
- OCR／載具自動去重：先比發票號碼，再比日期、金額與商家；載具取代 OCR
...
即使不設定外部服務，手動記帳、本機細分類與截圖 OCR 都可使用。載具同步及 Gemini AI 分類需要部署自己的 Google Apps Script 安全代理，完整步驟見 [載具與 AI 代理設定](./docs/invoice-carrier-setup.md)。

## 資料與隱私

帳本資料只存在瀏覽器 `localStorage` 的 `hukeep_personal_state_v1`，不會送往原 app。截圖 OCR 在裝置內完成；只有主動使用載具同步或 AI 複判時才會連到你部署的私人代理。載具驗證碼與代理通行碼不會儲存在瀏覽器。口語記帳在瀏覽器支援時優先嘗試裝置內辨識；不支援時可直接輸入文字，部分瀏覽器的語音辨識可能使用其線上服務。請定期用「備份與資料」匯出 JSON。
